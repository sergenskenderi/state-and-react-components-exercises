...
*/
/* STEP 0 
   Added useState from react library
   Sergi - 17 May 2021 */
import React , {useState} from 'react'; 

export default function Counter() {
  /* STEP 1 
   created const variable count and setCount to change the count variable value
  also initialized count = 0 
     Sergi - 17 May 2021*/
  const [count,setCount] = useState(0);

  const increment = () => {
    /* STEP 4 
    increase count by 1 using setCount
    Sergi - 17 May 2021 */
    setCount(count + 1);
  };
  const decrement = () => {
    /* STEP 5 
    decrease count by 1 using setCount
    Sergi - 17 May 2021 */
    setCount(count - 1);
  };
  const reset = () => {
    /* STEP 6 
    reset ( set count to 0 ) using setCount 
    Sergi - 17 May 2021 */
    setCount(0);
  };

  /* STEP 2 
     set the color of the text from the count number
    Sergi - 17 May 2021 */
  const style = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
  ...
  return (
    <div className='widget-counter container'>
      ...
      <div id='count' style={style}>
        {/* STEP 3 
         set the text getting from the count variable
        Sergi - 17 May 2021 */}
